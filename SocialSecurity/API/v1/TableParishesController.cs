...
namespace SocialSecurityAPI.API.v1
{
    ...
    [ApiController]
    public class TableParishesController : ControllerBase //ODataController
    {
        ...
        public TableParishesController(SocialSecurityDbContext dbContext, ILogger<TableParishesController> logger, IDateTimeUtc dateTimeUtc)
        {
            _context = dbContext;
            _logger = logger;
        ...
        {
            var tableResults = await _context.TableParishes.Select(t => new GeneralTable() { ID = t.ID, Code = t.Code, ShortDescription = t.ShortDescription, LongDescription = t.LongDescription }).ToListAsync();
            return tableResults;
        }
        ...
        {
            var tableResults = await _context.TableParishes.FindAsync(id);
        ...
        public async Task<ActionResult> Post([FromBody] GeneralPostTable generalPostTable)
        {
            ...
            try
            {
                _context.TableParishes.Add(table);
                await _context.SaveChangesAsync();
            }
            catch (Exception ex)
            {
                _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.errorSavingTableData, "Parishes", userData.UserName, ex.Message);
                throw;
            }
        ...
        public async Task<IActionResult> Put([FromODataUri] long key, [FromBody] GeneralGetTable generalGetTable)
        {
            ...
            var table = _context.TableParishes.Find(key);
            if (table == null)
            {
                return NotFound();
            }
            ...
            try
            {
                _context.TableParishes.Update(table);
                await _context.SaveChangesAsync();
            }
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.concurrencyError, "Parishes", userData.UserName, ex.Message);
                    throw;
                }
            }
            catch (Exception ex)
        ...
        {
            return _context.TableParishes.Any(e => e.ID == id);
        }
