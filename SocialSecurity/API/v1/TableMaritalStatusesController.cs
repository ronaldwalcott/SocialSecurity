...
namespace SocialSecurityAPI.API.v1
{
    ...
    [ApiController]
    public class TableMaritalStatusesController : ControllerBase //ODataController
    {
        ...
        public TableMaritalStatusesController(SocialSecurityDbContext dbContext, ILogger<TableMaritalStatusesController> logger, IDateTimeUtc dateTimeUtc)
        {
            _context = dbContext;
            _logger = logger;
        ...
        {
            var tableResults = await _context.TableMaritalStatuses.Select(t => new GeneralTable() { ID = t.ID, Code = t.Code, ShortDescription = t.ShortDescription, LongDescription = t.LongDescription }).ToListAsync();
            return tableResults;
        }
        ...
        {
            var tableResults = await _context.TableMaritalStatuses.FindAsync(id);
        ...
        public async Task<ActionResult> Post([FromBody] GeneralPostTable generalPostTable)
        {
            ...
            var table = new TableMaritalStatus();
            ...
            try
            {
                _context.TableMaritalStatuses.Add(table);
                await _context.SaveChangesAsync();
            }
            catch (Exception ex)
            {
                _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.errorSavingTableData, "MaritalStatuses", userData.UserName, ex.Message);
                throw;
            }
        ...
        public async Task<IActionResult> Put([FromODataUri] long key, [FromBody] GeneralGetTable generalGetTable)
        {
            ...
            var table = _context.TableMaritalStatuses.Find(key);
            if (table == null)
            {
                return NotFound();
            }
            ...
            try
            {
                _context.TableMaritalStatuses.Update(table);
                await _context.SaveChangesAsync();
            }
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.concurrencyError, "MaritalStatuses", userData.UserName, ex.Message);
                    throw;
                }
            }
            catch (Exception ex)
        ...
        {
            return _context.TableMaritalStatuses.Any(e => e.ID == id);
        }
