...
namespace SocialSecurityAPI.API.v1
{
    ...
    [ApiController]
    public class TableTaskStatusesController : ControllerBase //ODataController
    {
        ...
        public TableTaskStatusesController(SocialSecurityDbContext dbContext, ILogger<TableTaskStatusesController> logger, IDateTimeUtc dateTimeUtc)
        {
            _context = dbContext;
            _logger = logger;
        ...
        {
            var tableResults = await _context.TableTaskStatuses.Select(t => new GeneralTable() { ID = t.ID, Code = t.Code, ShortDescription = t.ShortDescription, LongDescription = t.LongDescription }).ToListAsync();
            return tableResults;
        }
        ...
        {
            var tableResults = await _context.TableTaskStatuses.FindAsync(id);
        ...
        public async Task<ActionResult> Post([FromBody] GeneralPostTable generalPostTable)
        {
            ...
            try
            {
                _context.TableTaskStatuses.Add(table);
                await _context.SaveChangesAsync();
            }
            catch (Exception ex)
            {
                _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.errorSavingTableData, "TaskStatuses", userData.UserName, ex.Message);
                throw;
            }
        ...
        public async Task<IActionResult> Put([FromODataUri] long key, [FromBody] GeneralGetTable generalGetTable)
        {
            ...
            var table = _context.TableTaskStatuses.Find(key);
            if (table == null)
            {
                return NotFound();
            }
            ...
            try
            {
                _context.TableTaskStatuses.Update(table);
                await _context.SaveChangesAsync();
            }
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.concurrencyError, "TaskStatuses", userData.UserName, ex.Message);
                    throw;
                }
            }
            catch (Exception ex)
        ...
        {
            return _context.TableTaskStatuses.Any(e => e.ID == id);
        }
