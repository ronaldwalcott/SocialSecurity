...
namespace SocialSecurityAPI.API.v1
{
    ...
    [ApiController]
    public class TablePostOfficesController : ControllerBase //ODataController
    {
        ...
        public TablePostOfficesController(SocialSecurityDbContext dbContext, ILogger<TablePostOfficesController> logger, IDateTimeUtc dateTimeUtc)
        {
            _context = dbContext;
            _logger = logger;
        ...
        {
            var tableResults = await _context.TablePostOffices.Select(t => new GeneralTable() { ID = t.ID, Code = t.Code, ShortDescription = t.ShortDescription, LongDescription = t.LongDescription }).ToListAsync();
            return tableResults;
        }
        ...
        {
            var tableResults = await _context.TablePostOffices.FindAsync(id);
        ...
        public async Task<ActionResult> Post([FromBody] GeneralPostTable generalPostTable)
        {
            ...
            try
            {
                _context.TablePostOffices.Add(table);
                await _context.SaveChangesAsync();
            }
            catch (Exception ex)
            {
                _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.errorSavingTableData, "PostOffices", userData.UserName, ex.Message);
                throw;
            }
        ...
        public async Task<IActionResult> Put([FromODataUri] long key, [FromBody] GeneralGetTable generalGetTable)
        {
            ...
            var table = _context.TablePostOffices.Find(key);
            if (table == null)
            {
                return NotFound();
            }
            ...
            try
            {
                _context.TablePostOffices.Update(table);
                await _context.SaveChangesAsync();
            }
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.concurrencyError, "PostOffices", userData.UserName, ex.Message);
                    throw;
                }
            }
            catch (Exception ex)
        ...
        {
            return _context.TablePostOffices.Any(e => e.ID == id);
        }
