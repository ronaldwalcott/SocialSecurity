...
namespace SocialSecurityAPI.API.v1
{
    ...
    [ApiController]
    public class TableDocumentTypesController : ControllerBase //ODataController
    {
        ...
        public TableDocumentTypesController(SocialSecurityDbContext dbContext, ILogger<TableDocumentTypesController> logger, IDateTimeUtc dateTimeUtc)
        {
            _context = dbContext;
            _logger = logger;
        ...
        {
            var tableResults = await _context.TableDocumentTypes.Select(t => new GeneralTable() { ID = t.ID, Code = t.Code, ShortDescription = t.ShortDescription, LongDescription = t.LongDescription }).ToListAsync();
            return tableResults;
        }
        ...
        {
            var tableResults = await _context.TableDocumentTypes.FindAsync(id);
        ...
        public async Task<ActionResult> Post([FromBody] GeneralPostTable generalPostTable)
        {
            ...
            var table = new TableDocumentType();
            ...
            try
            {
                _context.TableDocumentTypes.Add(table);
                await _context.SaveChangesAsync();
            }
            catch (Exception ex)
            {
                _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.errorSavingTableData, "DocumentTypes", userData.UserName, ex.Message);
                throw;
            }
        ...
        public async Task<IActionResult> Put([FromODataUri] long key, [FromBody] GeneralGetTable generalGetTable)
        {
            ...
            var table = _context.TableDocumentTypes.Find(key);
            if (table == null)
            {
                return NotFound();
            }
            ...
            try
            {
                _context.TableDocumentTypes.Update(table);
                await _context.SaveChangesAsync();
            }
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.concurrencyError, "DocumentTypes", userData.UserName, ex.Message);
                    throw;
                }
            }
            catch (Exception ex)
        ...
        {
            return _context.TableDocumentTypes.Any(e => e.ID == id);
        }
