...
namespace SocialSecurityAPI.API.v1
{
    ...
    [ApiController]
    public class TableEmploymentTypesController : ControllerBase //ODataController
    {
        ...
        public TableEmploymentTypesController(SocialSecurityDbContext dbContext, ILogger<TableEmploymentTypesController> logger, IDateTimeUtc dateTimeUtc)
        {
            _context = dbContext;
            _logger = logger;
        ...
        {
            var tableResults = await _context.TableEmploymentTypes.Select(t => new GeneralTable() { ID = t.ID, Code = t.Code, ShortDescription = t.ShortDescription, LongDescription = t.LongDescription }).ToListAsync();
            return tableResults;
        }
        ...
        {
            var tableResults = await _context.TableEmploymentTypes.FindAsync(id);
        ...
        public async Task<ActionResult> Post([FromBody] GeneralPostTable generalPostTable)
        {
            ...
            var table = new TableEmploymentType();
            ...
            try
            {
                _context.TableEmploymentTypes.Add(table);
                await _context.SaveChangesAsync();
            }
            catch (Exception ex)
            {
                _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.errorSavingTableData, "EmploymentTypes", userData.UserName, ex.Message);
                throw;
            }
        ...
        public async Task<IActionResult> Put([FromODataUri] long key, [FromBody] GeneralGetTable generalGetTable)
        {
            ...
            var table = _context.TableEmploymentTypes.Find(key);
            if (table == null)
            {
                return NotFound();
            }
            ...
            try
            {
                _context.TableEmploymentTypes.Update(table);
                await _context.SaveChangesAsync();
            }
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    _logger.LogError(LoggingEvents.TableConfiguration, LoggingErrorText.concurrencyError, "EmploymentTypes", userData.UserName, ex.Message);
                    throw;
                }
            }
            catch (Exception ex)
        ...
        {
            return _context.TableEmploymentTypes.Any(e => e.ID == id);
        }
